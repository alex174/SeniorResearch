// The Santa Fe Stock Market -- Implementation of NameTree class
...
// characters long, where MAXCHARS is a compile-time parameter.
// Each node in the tree may have one or more "children".  A node
...
// at most one descending from the root node and possibly others descending
// from orphan nodes.  For multiple root nodes, use multiple instances of
// this class.
//
...
//
// Every node has a null-terminated name stored in "name".  This is from 1 to
// MAXCHARS characters long (besides the \0) for leaf and branch nodes, and
// empty ("") for free nodes and the root node.  name[MAXCHARS] is always '\0'.
//
...
// Parameters
//
// MAXCHARS sets the maximum number of characters in a name.
//
